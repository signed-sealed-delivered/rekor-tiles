...
import (
	...
	gcpSQLDriver "github.com/sigstore/rekor-tiles/v2/internal/tessera/gcpcloudsql"
...
var serveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if !readOnly {
			driverConfig := gcpSQLDriver.DriverConfiguration{
				GCPCloudSQLDSN:      viper.GetString("gcp-cloudsql-dsn"),
			...
			tesseraDriver, persistentAntispam, err := gcpSQLDriver.NewDriver(ctx, driverConfig)
			if err != nil {
...
func init() {
	...
	// gcp configs
	serveCmd.Flags().String("gcp-cloudsql-dsn", "", "CloudSQL database URI")
